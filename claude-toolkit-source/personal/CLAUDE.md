...
- **In-session waiting: background sleep, not foreground.** Foreground leading sleeps are harness-blocked, BUT `Bash("sleep 110", run_in_background: true)` notifies the MAIN agent on completion (the channel sub-agents lack). So you CAN wait for CI green in one flow: background-sleep ≈ CI duration → `get_check_runs` → if `ci`+`desktop-ci` green, merge — no need to bounce to the user. Cross-turn polling (user's next message = interval) stays the fallback. Verified live: enables hands-off merge-on-green.

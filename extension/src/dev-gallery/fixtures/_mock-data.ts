...
const baseSection = (
  ...
  summary,
  key_terms: [
    { term: '事前確率', definition: 'データを観測する前の信念の度合い', ts: ts + 5, from: 'transcript' as const },
    { term: '事後確率', definition: 'データを観測した後の更新された信念', ts: ts + 12, from: 'transcript' as const },
    { term: '尤度', definition: '観測データが特定の仮説のもとで得られる確率', ts: 0, from: 'inferred' as const },
  ],
  examples: [
    { text: '医療検査で陽性が出ても、病気である確率は意外と低いことがある', ts: ts + 18, from: 'transcript' as const },
  ],
  points: [
    { text: 'ベイズの定理は条件付き確率を逆向きに扱う', ts: ts + 25, important: false, from: 'transcript' as const },
    { text: '事前確率の選び方が結果に大きく影響する', ts: ts + 40, important: withImportant, from: 'transcript' as const },
    { text: '対数尤度を使うと数値的に安定する', ts: ts + 55, important: false, from: 'transcript' as const },
    { text: 'したがって: 事前確率が極端に低い場合、検査精度だけでは確定診断に不十分', ts: 0, important: false, from: 'inferred' as const },
  ],

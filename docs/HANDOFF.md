...
**Last updated**: 2026-06-11 PM (q8_0+dump #111/#113 merged → v0.1.4. **First real-use retest FAILED**: 17.4-min interview (Claude kept open) finalize 3× prefill-stalled (zero output) → q8_0 alone insufficient — large-prompt prefill dies under memory pressure. Founder chose **adaptive 3B/1B + multi-chunk** (L-effort infra build — models.json single-path, 1B unbundled, no RAM detection). STT chunk-boundary cutting confirmed; noise mild in this quiet recording. **Next: fresh session design→plan→SDD; split a small multi-chunk-threshold-down PR first.** Forensics: memory v2_track2_17min_retest_failure_2026-06-11.)
...
| **TRACK 2 — long-input finalize reliability** | q8_0 landed but INSUFFICIENT; adaptive 3B/1B chosen (2026-06-11 PM) | Memory pressure confirmed root cause. q8_0 KV cache (#111, decode 3.3-3.5×, `grammar-ab.ts`) + debug dump (#113) → v0.1.4. **First real-use retest FAILED**: 17.4-min interview (Claude open) finalize 3× prefill-stalled at 60s with ZERO output — large-prompt (~7000-tok) prefill can't produce a first token under 8GB swap thrash; stall-recovery's reload discards warm pages, worsening it. q8_0 fixed decode-time KV, not prefill-under-pressure. Founder chose **adaptive 3B/1B (by available RAM) + multi-chunk**. Scope = L: models.json holds a single llmPath (no model-id/multi-model), 1B only in dev `~/.lisna-test-models`, no runtime RAM detection, 1B interview/brainstorm quality unverified. 1B profile (`llama-3.2-1b-q4-km`) already in `profiles.ts`. **Multi-chunk (lower `recommendedChunkTokens`) is a separable small first step** — keeps 3B quality, reuses tested interview merge. Full forensics: memory v2_track2_17min_retest_failure_2026-06-11. |

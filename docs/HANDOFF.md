...
**Last updated**: 2026-06-11 (decode-slowness root-caused = memory pressure, NOT grammar — q8_0 KV cache landed, decode 3.3×; PR #88 sanitizer fired in production for the first time (sanitized=2); **founder ~21-min quiet-machine retest on the repackaged app is the next deliverable** — gate: stalls 0 + finalize wall < 60s)
...
| **TRACK 2 — long-input decode latency** | Root-caused + fix landed (2026-06-11) | The earlier "NOT a memory swap" read was WRONG — it IS memory pressure: 16K fp16 KV (1.8GB) + 2GB weights on 8GB evicts decode pages mid-generate (founder retest 2026-06-11: 2 stall→restart cycles = 178s of a 212s finalize; grammar overhead measured at only ~1.5×, `desktop/scripts/grammar-ab.ts`). Fix: q8_0 KV cache (`feat/track2-kv-q8-cache`) → decode 3.3-3.5× (production-grammar 4.7→14.2-19.3 tok/s same machine). Remaining gate: founder ~21-min quiet-machine retest on repackaged app → stalls 0 + wall < 60s (the 1-min target). 1B fallback + output caps deliberately deferred unless that retest fails. |

...
**Last updated**: 2026-05-27 (v2 Plan 2 Foundation EXECUTED + Plans 2-7 written + Spike 0.2 Path F + lanes map)
...
| `9309b27` | **Session handoff** — 3 new `/learn` rules (dispatch trust-but-verify, dispatch send artifact ref, test-headers pair-update), decision record `2026-05-27-spike-0.1-amendment-1-n5-envelope`, REFACTOR_BACKLOG 3 new items, HANDOFF top date + §2 + §8. |

### What landed since (2026-05-27 evening, v2 Plan 2 Foundation + plan suite)

Continuation of the same v2 desktop track. Branch `spec/v2-note-creation-design`
advanced from `9309b27` to `0b1fbca` (29 commits). Independent of the extension
freeze.

| Commit | What |
|---|---|
| `27495ee` | **Spike 0.2 Path F (1B re-spike) — latency PASS, quality FAIL.** Per Path E controller recommendation. Same Spike 0.2 rig, `SPIKE_LLM_MODEL_PATH` swapped to `Llama-3.2-1B-Instruct-Q4_K_M.gguf`. Results across i=0/1/2: 21.4 s / 14.3 s / 80.0 s wall (mean of typical 17.8 s — Path E projection 22-25 s confirmed; 1 of 3 ran into 4095-token generation runaway → invalid JSON). Latency dimension **PASS** (1.86-3.23× faster than 3B on well-behaved runs, gen 14-16 ms/tok vs 3B's 38-67 ms/tok). Quality dimension **FAIL** (slot emergence 0/3 vs target ≥1/3; no formula extras emitted where 3B parroted `E=mc²` from the prompt). **Founder gate now open**: (a) 1B + Plan 6 prompt-v2 re-spike (if prompts can lift slot emergence ≥1/3), OR (b) 3B-as-default + Path G (`.max(N)` schema bounds + lower `MAX_TOKENS`) for tail-risk runaway suppression. Path G elevated from 1B stack-on → either-way production safety (Run 2 invalid-JSON failure mode is a real risk regardless of default). Plan 6 prompt engineering escalated from optional polish → load-bearing on the 1B branch. Memo: `desktop/spikes/phase-0/02-3b-lecture-grammar/decision-0.2-path-f.md`. |
| `675479b` / `89e664f` / `a704afc` / `761cd36` / `2f84af0` / `c1ca3c7` | **Plans 2-7 WRITTEN** via `superpowers:writing-plans`, pushed. Plan 2 Foundation (19 tasks, 2861 LOC), Plan 3 Lecture pipeline (17 tasks, 2053 LOC, first family E2E), Plan 4 Diarization integration (22 tasks, 2836 LOC), Plan 5 Meeting family (14 tasks, 1255 LOC, deterministic merge), Plan 6 Interview/Brainstorm/merge-LLM (19 tasks, 4979 LOC), Plan 7 Eval harness (30 tasks, 4693 LOC). Plan 2 = impl entry point (no fixture deps). Plan 7 = parallel-safe with any plan via PipelineRunner injection. Plan 4 Phase A = design-only (no founder fixture dep yet). Plans 3/5/6 live in `docs/superpowers/plans/2026-05-27-v2-plan-{3,5,6}-*.md`. |
| `f0b5a36` → `b2fb294` (19 commits) | **Plan 2 Foundation EXECUTED via SDD.** All 19 plan tasks landed + 1 narrowing chore (`ffe567f` TS strict-undefined). Each task ran the full SDD loop: implementer (sonnet/haiku) → spec-compliance reviewer → code-quality reviewer → both APPROVED → commit. Then a final holistic reviewer over the whole branch range. Architectural shape Plan 3+ inherits: (1) `@shared/note-schema` barrel — POST_DECODE_MARKER_DESCRIPTION, postDecodeOnly, Provenance / SpeakerRef / NoteBase / TranscriptSegment / SessionTranscript / GenerationTelemetry Zod schemas, estimateTokens (extended CJK), chunkTranscript, computeProvenance + ProvenanceConfig, hydratePostDecode, zodToGbnf. (2) `@shared/families` — FamilyDefinition&lt;T extends NoteBase&gt;, MergeStrategy, familyRegistry skeleton, registerFamily (throws on duplicate), util/prompts (PromptVariant + selectPromptVariant), util/slot (SlotDefinition). (3) `@shared/models/profiles` — ModelProfile + one entry `llama-3.2-3b-q4-km` (n\_ctx=16384, chunkTokens=8000, dormant-bos fix, ramBudgetMB=3072). (4) `@shared/pipeline-hooks` — 7 hooks (afterTranscribe / beforeDiarize / afterDiarize / beforeChunk / afterLLM / afterValidate / afterMerge). (5) `desktop/src/main/sidecar/grammar-call.ts` (main-process only, NOT in shared barrel) — `callWithGrammar` retry loop (`baseSeed + (attempt-1)*100`, JSON.parse / Zod / generator-throw catch, per-attempt `{seed, latencyMs, ok, reason}` telemetry), `makeSidecarGenerator(client)` factory, `GrammarCapableSidecar` interface (Plan 3 C++ side binds real `SidecarClient.generateWithGrammar`). Verification gate (Task 19): `pnpm --filter @lisna/desktop typecheck` → 0 errors; `pnpm --filter @lisna/desktop test` → 306/313 pass + 5 skipped + 2 pre-existing sidecar-dyld failures (UNRELATED to Plan 2; fix via `/lisna-sidecar-rebuild` skill per `pitfalls.md (sidecar)` rule). Final reviewer APPROVED ready-to-merge with 2 minor Plan 3 forward-compat notes (carry-forward as Plan 3 Task 1 pre-step + renderer process-split decision). |
| `252b053` | **3 new `/learn` rules from Plan 2 SDD execution.** (1) `testing.md (regression-fixture)` — regression test fixtures MUST be empirically verified to FAIL on the buggy code BEFORE the fix lands (Plan 2 Task 8 I-3: spec fixture 60-char segs / budget=3000 produced a soft boundary far from the planted silence and passed both before and after the fix; implementer recalibrated to 150-char / 9000 to expose the overflow). (2) `pitfalls.md (vitest-scope)` — `pnpm vitest run` at `desktop/` root WITHOUT a path filter picks up `spikes/phase-0/01-zod-to-gbnf/round-trip.test.ts` (hardware-gated real-LLM, ~16 min). For routine gates use `pnpm vitest run src/` or explicit paths. (3) `workflow.md (dispatch)` — SDD implementer prompts MUST include an explicit completion contract ("do not report back until the commit lands; early status = BLOCKED escalation"). Without it subagents (sonnet/haiku) report DONE after a failing-test step and never execute impl/commit (Plan 2 Tasks 2+3 each wasted ~10 min on truncate-then-resume; Tasks 4-18 with the contract completed one-shot). |
| `e8b9ef5` | **Multi-session isolation map (`.claude/lanes.md`).** Declares directory ownership for each Claude session lane (AI infra / Spec-Docs / App design / Eval / Web / Backend / Extension-frozen) + shared seams (package.json, lockfile, `.github/workflows/`, CLAUDE.md, PRD, rules, lanes itself) + cross-lane tagging convention (`Cross-lane:` git trailer, parallels `Co-Authored-By:`). Machine-parseable block at file bottom; pre-commit hook section 7 reads it and SOFT-warns when staged files fall outside the current worktree's owned set. Enables parallel sessions without silent drift on the same file. |
| `0b1fbca` | **Plan 3 fork-checkpoint after Phase A (Task 3).** Plan 3 (Lecture pipeline) updated with an explicit fork marker after Phase A Task 3 so the app-design lane worktree can pick up renderer work (Recording.tsx → NoteView) once main-process contracts freeze. Spec-docs lane edit; ai-infra lane continues main-process tasks in parallel. |

#### Plan 2 → Plan 3 carry-forwards (forward-compat notes from final reviewer)

1. **`families/index.ts` re-exports** — `PromptVariant`, `SlotDefinition`, `selectPromptVariant` are imported by `FamilyDefinition` but not re-exported from the families barrel. Plan 3 authors would otherwise need 3-path imports. Add as Plan 3 Task 1 pre-step.
2. **`FamilyDefinition.renderer` is a React `ComponentType`** — Plan 3 is mostly main-process work (orchestrator + IPC) but `renderer` field is renderer-process. Decide the loading-path before Plan 3 registers the first live family (Lecture): split FamilyDefinition by process? Lazy-load via dynamic import? Two-stage registration? **App-design lane gate.**

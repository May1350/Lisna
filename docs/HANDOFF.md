...
0. **v2 note pipeline: C++ engine LANDED — end-to-end now blocked on upstream Zod/GBNF gaps.** Branch `worktree-feat-cpp-grammar-gen` (off `origin/main` `6fa795f`) carries 8 implementation commits: C++ `GenOpts` gains `grammar`+`seed`; `LlamaEngine::generate` becomes `bool` with a grammar-first sampler + `opts.seed`-driven `dist`; `json_protocol` parses + shape-guards grammar/seed and stream-errors on grammar setup failure; TS IPC `generate` request carries optional `grammar`/`seed`; `makeGrammarSidecar` adapter implements `GrammarCapableSidecar` against `SidecarClient`; `ipc.ts::getCurrentSession` wires it (placeholder cast dropped); `desktop/eval` gains `fixtureToSessionTranscript` + `offline-3b` runner (family-guarded, counting-proxy retryAttempts) + `smoke-ja-mini` plumbing fixture + env-gated real-run test. C++ gtests 40/40, desktop typecheck+lint+vitest 274/274 green. Real-3B gate **ran** (24 s, single call, JSON parsed) but **failed** at `runPostDecodePipeline → schema.parse` on empty `sections[0].heading`. The plan's documented `common_sampler` fallback is NOT applicable — the gap is upstream of the sampler. Two new P0 follow-ups (must close before the gate can pass):

   - **P0a — Zod→GBNF: propagate `.min(N)` for string-shaped fields (and similar minimum constraints).** The Spike-0.1 grammar permits empty strings where the family schemas require non-empty. Fix the generator (likely under `desktop/src/shared/...` — exact path TBD) to emit non-empty GBNF repetition (`char+`) for `z.string().min(N>=1)`. Re-run the offline-3b smoke gate after the fix lands. `common_sampler` would NOT help — both samplers emit grammar-valid empty strings.

   - **P0b — chunked-note: thread real family schema into `callWithGrammar` (or route post-decode ZodError back into retry).** `orchestrator.ts` passes `z.unknown()` so the per-attempt validator accepts anything; the Spike-0.1 retry-on-Zod-rejection contract never fires for the real schema. The real check runs AFTER, in `runPostDecodePipeline`, where the `ZodError` just propagates with no recovery. Either pass the family schema to `callWithGrammar` directly, OR catch the post-decode `ZodError` and trigger a fresh-seed retry through `callWithGrammar`.

   Diagnosis + decision recorded in `docs/superpowers/specs/2026-05-28-cpp-grammar-constrained-generation-design.md` §5 Gate outcome. Branch is shippable as-is (no regressions, sampler-side correctness validated by the structurally-clean JSON output); it just doesn't *unblock the pipeline yet*. Founder decides: merge the 8 commits as a partial step + tackle P0a/P0b next, or hold the branch until P0a/P0b are also ready to ship together.

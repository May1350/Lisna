...
- [P3] **backgroundJob chip: don't lose a completed-unopened result on a new generation** — Phase 2 review (medium): starting a new generation (quick-transcript span / note pick / History regen) while a prior job's chip is `done`/`error` but not yet opened silently overwrites it (genBusy only guards `running`). The result is recoverable (transcript → History; note → dump result.json), but the chip's one-click "開く" is lost. Founder UX call: a chip queue / multiple chips / auto-persist notes vs accept History recovery. Touches: `desktop/src/renderer/App.tsx` (backgroundJob model). Effort: M.
- [P3] **Quick-transcript span: byte-position bounds, not renderer wall-clock** — Phase 2 review (low): the span uses renderer `elapsedSec` (wall-clock) mapped to WAV bytes assuming 1s==32000 captured bytes. Audio drops/clock-skew would misalign the slice. Acceptable for a rough quick transcript; if the founder finds it off, track the live `_audioWriter.dataBytes` cursor at span start/stop (needs a main→renderer byte-cursor push or an IPC). Touches: `desktop/src/main/ipc.ts`, `desktop/src/renderer/routes/Recording.tsx`. Effort: M.
- [P2] **Orphaned-WAV recovery (background-generation Phase 2)** — under background generation the capture is freed at pick (spec §4.2), so a generation that fails BEFORE the dump transcript is written (only STT double-stall hits this) leaves the recorded WAV in `userData/audio-captures/` with no in-app re-transcribe path (bytes safe; UI-unreachable). Old code preserved it via P0-3 live-retry. Fix: persist the wavPath in the dump at step A + a History "re-transcribe this audio" path that re-runs STT from the orphaned WAV. Pairs with the Phase-2 slice work (also handles WAV-by-path). Independent review flagged 2026-06-30; comments corrected in `ipc.ts` onSessionSettled. Touches: `desktop/src/main/ipc.ts` (runGenerationContext step A), `session-debug-dump.ts`, `session-dump-reader.ts`, `dump-finalize-context.ts`. Effort: M.

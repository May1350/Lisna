...
inferred の見つけ方 (self-check):
1. transcript を読み、講師が使った術語を列挙する
2. その術語のうち、transcript に定義が出てこないものを特定する
3. それが学部 1〜2 年生に自明でないなら → key_terms に from:'inferred' で追加
4. 「AだからB」という論理で、AとBの間のステップが欠落していれば → points/argument_chain に from:'inferred'

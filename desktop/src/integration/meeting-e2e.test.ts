/**
 * Hardware-gated Meeting E2E test.
 *
 * Runs the full Meeting pipeline (transcript → diarization-fallback → chunk →
 * grammar-call → post-decode → merge → final parse) against a REAL LLM.
 * Gated behind `LISNA_LLM_INTEGRATION=1` env so CI never invokes it.
 *
 * STATUS (Plan 5 Task 12): SCAFFOLD ONLY. The production
...
import { MeetingNoteSchema } from '../shared/families/meeting/schema';
...
import '../shared/families/meeting/core';   // side-effect: register MeetingFamilyCore
...
describe.skipIf(!HARD_GATED)('Meeting E2E (real LLM, LISNA_LLM_INTEGRATION=1)', () => {
  it(
    'full pipeline produces a valid MeetingNote from a 2-speaker fixture (fallback path)',
    async () => {
      // Load + validate the fixture against the v2 SessionTranscript schema.
      const fixturePath = resolve(__dirname, 'fixtures/meeting-2spk.transcript.json');
      ...
      // Exercise the diarization-disabled fallback path: all segments collapse
      // to speaker 0 and SINGLE_SPEAKER_WARNING is injected into the note.
      const result = await finalizeMeeting({
        sessionId: 'e2e-test-meeting-2spk',
        transcript,
        sidecar,
      ...
      expect(() => MeetingNoteSchema.parse(result.note)).not.toThrow();
      expect(result.note.family).toBe('meeting');

      // Fallback path must surface the single-speaker degradation warning.
      expect(result.note.validation_warnings).toContain(SINGLE_SPEAKER_WARNING);

      // Sanity: at least some output fields are populated (bounds allow 0 for decisions).
      expect(
        result.note.decisions.length + (result.note.next_steps?.length ?? 0),
 ...
 *   return adapter;
 *
 * NOTE: duplicated from lecture-e2e.test.ts (2 call sites). Plan 6 may
 * extract a shared helper when the 3rd family E2E lands (DRY rule: 3+ call sites).
 */

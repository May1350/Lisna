...
| 0.2 3B Lecture | Zod validates + ≥1 slot emergence + < 30s per chunk | **MIXED** (2026-05-27: 3/3 Zod-valid · 3/3 ≥1 slot (counts 4/2/4, all formula) · latency p50 = 90 s, p90 = 99 s — 3× over 30 s budget on M3/8GB · 11.3K transcript chars / 13.4K prompt chars / 8 K JA tokens, at top of §2.3 ~8K chunk budget) | **Path E result (2026-05-27):** cost split MIXED (mean PE 48% / EV 52%, neither phase ≥65% dominance). Path B alone insufficient. Forward = **Path F (1B Q4_K_M re-spike)** + Path G (output `.max(N)` bound) stack-on. Plan 6 implication: 1B as ≤ 12 GB default capability floor. Memos: `02-3b-lecture-grammar/decision-0.2-latency.md` (paths A-E ladder), `decision-0.2-path-e.md` (timing data + Path F rationale). |
